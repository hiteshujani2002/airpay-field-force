-- Update RLS policy to allow client admins to see CPV agents assigned to their company's merchants
...
USING (
  ...
  ((get_current_user_role() = 'lead_assigner'::app_role) AND (role = 'cpv_agent'::app_role) AND (created_by_user_id = auth.uid())) OR
  (user_id = auth.uid())
);